...
class GAU(nn.Module):
    def __init__(self, net_params):
        ...
        self.d_k = hidden_dim // self.num_heads  # 256/4=64
        self.e = int(self.d_k * net_params['expansion_factor'])  # 64*2=128
        self.s = net_params['s'] // self.num_heads # 128/4=32
        self.uv = nn.Linear(hidden_dim, 2 * self.num_heads * self.e + self.num_heads * self.s)
        self.weight = nn.Parameter(torch.randn(2, self.s))
        self.bias = nn.Parameter(torch.zeros(2, self.s))
        self.o = nn.Linear(self.num_heads * self.e, hidden_dim)
        ...
        self.act_fn = ACT2FN[net_params['hidden_act']]
    ...
    def forward(self, x, attention_mask=None, output_attentions=False, causal=False):
        ...
        uv = self.uv(x).view(seq_len, -1, self.num_heads, 2 * self.e + self.s)
        uv = uv.permute(2, 0, 1, 3)  # switch head and sequence dimensions
        ...
        if self.softmax:
            kernel = qk + attention_mask.unsqueeze(1)
            kernel = F.softmax(kernel, dim = -1)

        x = u * torch.einsum("bhnm,bhme->bhne", kernel, v)
        x = x.permute(1, 2, 0, 3).reshape(seq_len, -1)  # flatten the head dimension
        print(x.shape)
        x = self.o(x)
