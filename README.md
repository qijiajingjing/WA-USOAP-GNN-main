# Weak-Attention and Unified SOAP Descriptors Graph Neural Network (WA-USOAP-GNN)
...
#### Dataset
The main script to train model is `train_folder.py`. A user needs at least the following info to train a model: 
1) `id_prop.csv` with name of the file and corresponding value, 
2) `config_example.json` a config file with training and hyperparameters.

训练模型的主脚本是“train_folder.py”。用户至少需要以下信息来训练模型：
1）`id_prop.csv` 带有文件名和相应值
2）`config_example.json 是一个包含训练和超参数的配置文件。

Users can keep their structure files in `POSCAR`, `.cif`, `.xyz` or `.pdb` files in a directory. 
In the examples below we will use POSCAR format files. In the same directory, there should be an `id_prop.csv` file. 
用户可以将其结构文件保存在`POSCAR`,`.cif`，`.xyz`或`.pdb`文件。
在下面的示例中，我们将使用POSCAR格式文件。在同一目录中，应该有一个`id_prop.csv`文件。

In this directory, `id_prop.csv`, the filenames, and correponding target values are kept in `comma separated values (csv) format`.
在此目录`id_prop。csv`中，文件名和相应的目标值以“逗号分隔值（csv）格式”保存。
...
这是训练JARVIS-DFT数据库中50种材料的OptB88vdw带隙的示例。该示例使用[generate_sample_data_reg.py]创建(https://github.com/usnistgov/alignn/blob/main/alignn/examples/sample_data/scripts/generate_sample_data_reg.py)脚本。用户可以修改50多个数据的脚本，或以此格式创建自己的数据集。有关可用数据集的列表，请参见[数据库](https://jarvis-tools.readthedocs.io/en/master/databases.html).
...
在80:10:10中分割的数据集作为训练验证测试集（由“train_ratio，val_ratio和test_ratio”控制）。要更改分割比例和其他参数，请更改[config_example.json](https://github.com/usnistgov/alignn/blob/main/alignn/examples/sample_data/config_example.json)文件。如果用户希望在某些数据集上训练，并在另一个数据集上进行val/test，请在“config_example”中手动设置“n_train”、“n_val”、“n _test”。json`，并将“keep_data_order”设置为True，以便禁用随机洗牌。
...
现在，对模型进行如下训练。请将“config_example.json”中的“batch_size”参数增加到32或64，以便进行一般培训。
...
While the above exfample is for regression, the follwoing example shows a classification task for metal/non-metal based on the above bandgap values. We transform the dataset
...
虽然上面的示例是用于回归的，但下面的示例显示了基于上述带隙值的金属/非金属分类任务。我们转换数据集
基于0.01eV的阈值（由参数‘classiication_threshold’控制）将其转换为1或0，并训练类似的分类模型。目前，该脚本只允许二进制分类任务。
```
...
用户可以尝试使用多个示例脚本来运行多个数据集（如JARVIS-DFT、Materials项目、QM9_JCTC等）。
查看[alignn/scripts/train_*.py](https://github.com/usnistgov/alignn/tree/main/alignn/scripts)文件夹。这主要是为了使培训更加自动化，而不是制作文件夹/csv文件等。
这些脚本自动从[Databases in jarvis tools]下载数据集(https://jarvis-tools.readthedocs.io/en/master/databases.html)并训练几个模型。请确保在脚本中指定特定的排队系统详细信息。

<a name="pretrained"></a>
Using pre-trained models  使用预先训练的模型
-------------------------
...
使用JARVIS-DFT数据集训练模型预测每个原子的形成能量的示例如下所示：
```
